...
madc2vcf_all <- function(madc,
  ...
  vmsg("madc              : %s", verbose = verbose, level = 1, madc)
  vmsg("botloci_file      : %s", verbose = verbose, level = 1, botloci_file)
  vmsg("hap_seq_file      : %s", verbose = verbose, level = 1, if (is.null(hap_seq_file)) "NULL" else hap_seq_file)
  vmsg("markers_info      : %s", verbose = verbose, level = 1, if (is.null(markers_info)) "NULL" else markers_info)
  vmsg("n.cores           : %s", verbose = verbose, level = 1, n.cores)
  vmsg("alignment_score_thr       : %s", verbose = verbose, level = 1, alignment_score_thr)
  vmsg("rm_multiallelic_SNP       : %s", verbose = verbose, level = 1, rm_multiallelic_SNP)
  vmsg("multiallelic_SNP_dp_thr   : %s", verbose = verbose, level = 1, multiallelic_SNP_dp_thr)
  vmsg("multiallelic_SNP_sample_thr: %s", verbose = verbose, level = 1, multiallelic_SNP_sample_thr)
  vmsg("add_others        : %s", verbose = verbose, level = 1, add_others)
  vmsg("others_max_snps   : %s", verbose = verbose, level = 1, if (is.null(others_max_snps)) "NULL" else others_max_snps)
  vmsg("others_rm_with_indels     : %s", verbose = verbose, level = 1, others_rm_with_indels)
  vmsg("out_vcf           : %s", verbose = verbose, level = 1, if (is.null(out_vcf)) "NULL" else out_vcf)
