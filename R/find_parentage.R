...
#'       \code{_2}, etc. for each tied pair.
#'   }
  ...
  if (method %in% c("best_male_parent", "best_female_parent", "best_match")) {
    ...
    results_list <- base::lapply(base::rownames(progeny_genos), function(progeny_id) {
      ...
      if (base::length(best_idx) == 0) {
        data.table::data.table(Progeny = progeny_id, Best_Match = NA,
                               Mendelian_Error_Pct = NA, Markers_Tested = NA)
      } else {
        data.table::data.table(
          Progeny             = progeny_id,
          Best_Match          = base::rownames(parent_genos)[best_idx],
          Mendelian_Error_Pct = base::round(percent_mismatch[best_idx], 2),
  ...
  if (method == "best_pair") {
    ...
    results_list <- base::lapply(progeny_candidates$ID, function(prog_id) {
      ...
      if (base::is.infinite(min_mismatch_val)) {
        return(data.table::data.table(Progeny = prog_id, Markers_Tested = 0))
      }
      ...
      num_to_report <- if (show_ties) base::nrow(best_pairs) else 1
      num_to_report <- base::min(base::nrow(best_pairs), num_to_report)
