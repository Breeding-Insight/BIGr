...
#' @param multiallelic_SNP_dp_thr numerical. If `rm_multiallelic_SNP` is FALSE, set a total depth by tag threshold to eliminate low frequency SNP allele. If the threshold does not eliminate the multiallelic aspect of the marker, the marker is discarded. This is likely to happen to paralogous sites.
...
get_OffTargets <- function(madc = NULL,
  ...
  vcf_body <- create_VCF_body(my_results_csv, n.cores = n.cores,
                              rm_multiallelic_SNP = rm_multiallelic_SNP,
                              multiallelic_SNP_dp_thr = multiallelic_SNP_dp_thr,
                              verbose = verbose)
...
#'
#' @import parallel
create_VCF_body <- function(csv, rm_multiallelic_SNP = TRUE, multiallelic_SNP_dp_thr = 10, n.cores = 1, verbose = TRUE){

  #csv <- my_results_csv
  cloneID <- split.data.frame(csv, csv$CloneID)

  clust <- makeCluster(n.cores)
  clusterExport(clust, c("merge_counts","rm_multiallelic_SNP", "multiallelic_SNP_dp_thr"))
  #clusterExport(clust, c("merge_counts"))
  vcf_tag_list <- parLapply(clust, cloneID, function(x) merge_counts(x, rm_multiallelic_SNP, multiallelic_SNP_dp_thr))
  stopCluster(clust)
    ...
    for(i in 1:length(by_pos)){
      ...
      if(length(unique(by_pos[[i]]$Alt)) > 1){ # If SNP is multiallelic
        ...
        if(rm_multiallelic_SNP){ # option to remove multiallelics
          rm <- rm + 1
          break()
        } else if(multiallelic_SNP_dp_thr > 0){ # If not removed, user can set threshold to remove low frequency alleles
          rm.idx <- which(apply(by_pos[[i]][,-c(1:7)], 1, sum) < multiallelic_SNP_dp_thr)
