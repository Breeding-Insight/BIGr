...
#' }
#'
#' @importFrom data.table fread fwrite rbindlist copy data.table := set as.data.table
#' @export
#'
#'
validate_pedigree <- function(pedigree_file, genotypes_file,
  ...
  results_list <- lapply(seq_len(nrow(pedigree)), function(i) {
    ...
    # Initialise per-parent and best-parent fields
    ...
    data.table(
      ...
      Sire_Hom_Error_Pct  = sire_error_pct,
      Dam_Hom_Error_Pct   = dam_error_pct,
      Best_Sire           = best_sire,
  ...
  for (i in seq_len(nrow(final_df))) {
    ...
    if (decision == "REMOVE_SIRE") {
      set(corrected_pedigree, which(corrected_pedigree$Progeny == prog_id), "Sire", 0L)
    } else if (decision == "REMOVE_DAM") {
      set(corrected_pedigree, which(corrected_pedigree$Progeny == prog_id), "Dam", 0L)
  ...
  if (fill_pedigree) {
    ...
    for (i in seq_len(nrow(final_df))) {
      ...
      decision <- final_df$Correction_Decision[i]
      if (decision == "REMOVE_SIRE") {
        set(filled_pedigree, which(filled_pedigree$Progeny == prog_id), "Sire", final_df$Best_Sire[i])
      } else if (decision == "REMOVE_DAM") {
        set(filled_pedigree, which(filled_pedigree$Progeny == prog_id), "Dam",  final_df$Best_Dam[i])
      } else if (decision == "REMOVE_BOTH") {
        set(filled_pedigree, which(filled_pedigree$Progeny == prog_id), "Sire", final_df$Best_Sire[i])
        set(filled_pedigree, which(filled_pedigree$Progeny == prog_id), "Dam",  final_df$Best_Dam[i])
      }
